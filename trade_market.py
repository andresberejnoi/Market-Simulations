'''
Code taken from my previous work on creating some simple tools for calculating tax liability with crypto.
I modified it to fit my needs here.

Original file: https://github.com/andresberejnoi/CryptoTaxTools/blob/main/tax_objects.py
...
class Lot(object):
    ...
    def cost_basis(self, val):
        self._cost_basis = decimal.Decimal(abs(val))
    ...
    def quantity(self, val):
        self._quantity = decimal.Decimal(abs(val))
    ...
    def date_purchased(self, date_val):
        self._date_purchased = pd.to_datetime(date_val)
    ...
    @date_sold.setter
    def date_sold(self, date_val):
        self._date_sold = pd.to_datetime(date_val)
...
class Pool(object):
    ...
    def show_lots(self):
        ...
        print(f"Number of lots: {len(self.lots)}")

    def lots_to_df(self):
        '''I want to make this function more efficient. For now, it is just a placeholder using a for loop.'''
        columns = ['Date purchased', 'Date Sold', 'Quantity', 'Cost Basis', 'Id']
        ...
        for lot in self.lots:
            purchased_dates.append(lot.date_purchased)
            cost_basis_list.append(lot.cost_basis)
            sold_dates.append(lot.date_sold)
            quantities.append(lot.quantity)
            ids.append(lot.id)

        _df_dict = {
            'Date Purchased': purchased_dates,
            'Date Sold'     : sold_dates,
            'Coin Amount'   : quantities,
            'Cost Basis'    : cost_basis_list,
            'IDs'           : ids,
        }
...
if __name__ == "__main__":
    pass   #I will eventually add some unit testing here